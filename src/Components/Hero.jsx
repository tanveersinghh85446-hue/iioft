...
function ChatBot() {
  ...
  const getBotResponse = (userMessage) => {
    ...
    if (lowerMsg.includes("course") || lowerMsg.includes("kya padhai") || lowerMsg.includes("subjects") || lowerMsg.includes("📚")) {
      return {
        text: "IIOFT mein ye popular courses available hain:\n\n🔹 Data Science & AI/ML\n🔹 Web Development (Full Stack)\n🔹 Digital Marketing\n🔹 Ethical Hacking & Cyber Security\n🔹 Cloud Computing (AWS, Azure)\n🔹 Python Programming\n\nSabhi courses mein:\n✅ Hands-on training\n✅ Industry projects\n✅ Global certification\n✅ 100% placement support\n\nKaunsa course detail mein jaanna chahenge?",
        options: ["💼 Placement details", "💰 Fees structure", "📞 Call karo mujhe", "📍 Nearest center"],
      };
    }
    if (lowerMsg.includes("placement") || lowerMsg.includes("job") || lowerMsg.includes("guarantee") || lowerMsg.includes("💼")) {
      return {
        text: "IIOFT ki placement record bahut strong hai! 🎯\n\n✅ 100% Placement Support\n✅ Top companies: Google, Microsoft, IBM, Accenture, Wipro, TCS, Infosys\n✅ Resume building & mock interviews\n✅ Dedicated placement team\n✅ IIOFT Alumni Network benefits\n\nAverage package: 4-8 LPA\nHighest package: 15+ LPA\n\nAap apply karna chahenge?",
        options: ["✅ Apply Now", "📚 Course details", "📞 Talk to counselor", "🌐 Visit website"],
      };
    }
    if (lowerMsg.includes("fees") || lowerMsg.includes("price") || lowerMsg.includes("cost") || lowerMsg.includes("kitna") || lowerMsg.includes("💰")) {
      return {
        ...
        options: ["📞 Call karo", "✅ Apply Now", "📚 Course details", "📍 Visit center"],
      };
    }
    if (lowerMsg.includes("center") || lowerMsg.includes("location") || lowerMsg.includes("kaha") || lowerMsg.includes("address") || lowerMsg.includes("📍")) {
      return {
        ...
        options: ["📞 Contact details", "✅ Apply Now", "💼 Placement info", "📚 Course details"],
      };
    }
    if (lowerMsg.includes("apply") || lowerMsg.includes("admission") || lowerMsg.includes("enroll") || lowerMsg.includes("join") || lowerMsg.includes("✅")) {
      return {
        text: "Great choice! 🎉 IIOFT mein admission bahut easy hai!\n\n📋 Eligibility:\n• Minimum 10+2 (most courses)\n• All backgrounds welcome\n\n📝 Apply kaise karein:\n1️⃣ Apply Now button click karein\n2️⃣ Form fill karein\n3️⃣ Hamari team 24hrs mein contact karegi\n\n🚀 Quick Contact:\n📞 +91 9560307098\n📧 info@iioft.co.in",
        options: ["📞 Call now", "💬 WhatsApp", "📧 Email", "📚 Course details"],
      };
    }
    if (lowerMsg.includes("contact") || lowerMsg.includes("call") || lowerMsg.includes("phone") || lowerMsg.includes("number") || lowerMsg.includes("📞")) {
      return {
        ...
        options: ["📞 Call karo", "💬 WhatsApp", "✅ Apply Now", "🏠 Back to main menu"],
      };
    }
    if (lowerMsg.includes("hi") || lowerMsg.includes("hello") || lowerMsg.includes("hey") || lowerMsg.includes("namaste")) {
      return {
        ...
        options: ["📚 Courses", "💼 Placement", "💰 Fees", "📍 Centers"],
      };
    }
    if (lowerMsg.includes("thanks") || lowerMsg.includes("thank you") || lowerMsg.includes("shukriya")) {
      return {
        text: "Aapka swagat hai! 😊\n\nKoi aur help chahiye to zaroor puchiye!\n\nIIOFT team ready hai aapki help karne ke liye! 🎓",
        options: ["📚 More about courses", "✅ Apply Now", "📞 Contact us", "🏠 Main menu"],
      };
    }
    return {
      text: "Main aapki query samajh nahi paya. 🤔\n\nAap ye topics ke baare mein pooch sakte hain:\n\n📚 Courses aur subjects\n💼 Placement aur job guarantee\n💰 Fees aur EMI options\n📍 Centers aur locations\n✅ Admission process\n📞 Contact details\n\nYa phir directly hamse contact karein:\n📞 +91 9560307098",
  ...
  const handleSend = (messageText = inputText) => {
    ...
    setTimeout(() => {
      ...
      const botMessage = { id: Date.now() + 1, text: botResponseData.text, sender: "bot", time: new Date(), options: botResponseData.options };
          ...
          >
            ...
            <div className="bg-linear-to-r from-blue-600 to-blue-700 p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-bold text-lg">IIOFT Assistant</h3>
                  <p className="text-blue-100 text-xs">Online • Replies instantly</p>
                </div>
              </div>
              <button onClick={() => setIsOpen(false)} className="text-white hover:bg-white/20 p-2 rounded-full transition-all">
              ...
              {messages.map((msg) => (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`flex gap-2 max-w-[80%] ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                    <div className={`${msg.sender === "bot" ? "bg-blue-100" : "bg-blue-600"} p-2 rounded-full h-8 w-8 flex items-center justify-center shrink-0`}>
                      {msg.sender === "bot" ? <FaRobot className="text-blue-600 text-sm" /> : <FaUserCircle className="text-white text-sm" />}
                    </div>
                    <div>
                      <div className={`${msg.sender === "user" ? "bg-blue-600 text-white rounded-tl-2xl rounded-bl-2xl rounded-br-sm" : "bg-white text-gray-800 rounded-tr-2xl rounded-br-2xl rounded-bl-sm shadow-md"} p-3 whitespace-pre-line text-sm leading-relaxed`}>
                        {msg.text}
                      </div>
                      {msg.options && msg.sender === "bot" && (
                        <div className="mt-2 space-y-2">
                          {msg.options.map((option, idx) => (
                            <motion.button key={idx} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} onClick={() => handleSend(option)} className="block w-full text-left px-3 py-2 bg-white border border-blue-200 text-blue-600 rounded-lg text-sm hover:bg-blue-50 hover:border-blue-400 transition-all">
                              {option}
                            </motion.button>
                          ))}
                        </div>
                      )}
                      <p className="text-xs text-gray-400 mt-1">{msg.time.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
            ...
            <div className="px-4 py-2 bg-gray-50 border-t border-gray-200 flex gap-2 justify-center">
              <a href="tel:+919560307098" className="flex items-center gap-1 text-xs bg-green-500 text-white px-3 py-1.5 rounded-full hover:bg-green-600 transition-all">
                <BsPhoneFill className="text-xs" /> Call
              </a>
              <a href="https://wa.me/919560307098" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs bg-green-600 text-white px-3 py-1.5 rounded-full hover:bg-green-700 transition-all">
                <BsWhatsapp className="text-xs" /> WhatsApp
              </a>
              <a href="mailto:info@iioft.co.in" className="flex items-center gap-1 text-xs bg-blue-500 text-white px-3 py-1.5 rounded-full hover:bg-blue-600 transition-all">
            ...
            <div className="p-4 bg-white border-t border-gray-200">
              <div className="flex gap-2 items-end">
                <input ref={inputRef} type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type your message..." className="flex-1 px-4 py-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" />
                <button onClick={() => handleSend()} disabled={!inputText.trim()} className="bg-blue-600 text-white p-3 rounded-full hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all">
                  <FaPaperPlane className="text-lg" />
                </button>
              </div>
              <p className="text-xs text-gray-400 text-center mt-2">Powered by IIOFT AI Assistant</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function LogoMarquee() {
  return (
    <div className="overflow-hidden py-10 bg-white">
      <motion.div className="flex gap-16" animate={{ x: ["0%", "-100%"] }} transition={MARQUEE_TRANSITION}>
        {[...LOGOS, ...LOGOS].map((logo, i) => (
...
export default function Hero() {
  ...
  return (
    <div>
      ...
      {/* ── HERO ── */}
      <section className="relative w-full h-screen flex items-center justify-center text-center overflow-hidden">
        <motion.img key={index} src={IMAGES[index]} initial={{ x: "100%" }} animate={{ x: "0%" }} exit={{ x: "-100%" }} transition={{ duration: 1 }} className="absolute w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/70" />
        <div className="relative z-10 px-6 max-w-4xl text-white">
          <motion.h2 initial={{ x: -100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 1 }} viewport={{ once: true }} className="uppercase tracking-widest text-sm md:text-base text-blue-400 mb-4">
            World's Best Institute
          </motion.h2>
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold leading-tight">
            International Institute of <br />
          ...
          <p className="mt-6 text-base sm:text-lg md:text-xl lg:text-2xl font-semibold drop-shadow-md">
            <span className="text-red-500">Think</span> <span className="text-yellow-400">Smart</span> | <span className="text-red-500">Learn</span> <span className="text-yellow-400">Smart</span>
            <br />
            <span className="text-yellow-400">Skill Up with </span>
            <span className="text-blue-400 font-bold">IIOFT</span>
          </p>
      ...
      {/* ── PROS ── */}
      <div className="flex items-center justify-center mt-12 md:mt-16 px-4 text-center">
        <motion.h2 initial={{ x: -100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 1 }} viewport={{ once: true }} className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold">
          The <span className="text-blue-600 font-serif">IIOFT</span> pro's
        </motion.h2>
      </div>
      <div className="py-8 md:py-16 px-4 md:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
          {PROS.map((item, i) => (
            <motion.div key={i} initial={{ y: 60, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: i * 0.2 }} viewport={{ once: true }} className="bg-blue-500 rounded-2xl p-4 md:p-8 shadow-lg text-center hover:scale-105 transition duration-300">
      ...
      <div className="py-16 md:py-20 mb-10">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-black max-w-xl mx-auto">A quick look at our achievements and growth in numbers.</p>
        </div>
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-blue-600 px-4">
          {COUNTER_LABELS.map(({ key, suffix, label }) => (
            <div key={key}>
      ...
      <section className="py-20 bg-blue-600">
        ...
        <div className="overflow-hidden mb-8">
          <motion.div className="flex gap-8" animate={{ x: ["0%", "-100%"] }} transition={{ repeat: Infinity, duration: 18, ease: "linear" }}>
            {[...PLACEMENT_LOGOS.slice(0, 10), ...PLACEMENT_LOGOS.slice(0, 10)].map((logo, i) => (
              <div key={i} className="min-w-45 bg-black rounded-2xl p-6 flex items-center justify-center shadow-lg">
                <img src={`/${logo}`} alt="company logo" className="h-12 md:h-16 object-contain" />
              </div>
            ))}
          </motion.div>
        </div>
        <div className="overflow-hidden">
          <motion.div className="flex gap-8" animate={{ x: ["-100%", "0%"] }} transition={{ repeat: Infinity, duration: 18, ease: "linear" }}>
            {[...PLACEMENT_LOGOS.slice(10), ...PLACEMENT_LOGOS.slice(10)].map((logo, i) => (
              <div key={i} className="min-w-45 bg-black rounded-2xl p-6 flex items-center justify-center shadow-lg">
                <img src={`/${logo}`} alt="company logo" className="h-12 md:h-16 object-contain" />
              </div>
            ))}
      ...
      {/* ── FAQ ── */}
      <section className="relative py-20 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('AB.jpeg')" }}>
        <div className="absolute inset-0 bg-black/50" />
        <div className="relative max-w-6xl mx-auto px-4">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center text-white mb-14">Frequently Asked Questions</h2>
          <div className="space-y-4">
            {FAQS.map((faq, i) => (
              <div key={i} className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden">
                <button className="flex justify-between items-center p-4 sm:p-6 w-full text-left" onClick={() => toggle(i)}>
                  <h3 className="text-base sm:text-lg font-semibold text-black">{faq.question}</h3>
                  <span className="text-red-600 text-xl font-bold">{openIndex === i ? "−" : "+"}</span>
                </button>
                {openIndex === i && (
                  <div className="p-4 sm:p-6 border-t border-gray-200 bg-gray-50">
      ...
      <div className="bg-blue-600 text-white px-6 md:px-12 lg:px-32 py-12">
        <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
          <div className="w-full lg:max-w-55 lg:shrink-0">
            ...
            <p className="text-sm text-blue-100 leading-relaxed">International Institute of Futuristic Technology (IIOFT) prepares students with practical skills and advanced knowledge.</p>
            <div className="mt-6">
          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-3 lg:contents gap-8">
            <div className="lg:shrink-0">
              ...
              <ul className="space-y-1.5">
                {LOCATIONS.map((loc, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-blue-100 cursor-pointer hover:text-black transition-colors duration-200" onClick={() => window.open(loc.link, "_blank")}>
                    <FaMapMarkerAlt className="text-blue-300 text-xs mt-1 shrink-0" /> {loc.name}
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="flex flex-col space-y-1.5">
                {[["Home", "/"], ["Contact", "/contact"], ["Course", "/course"], ["About", "/about"], ["Know More", "/know-more"]].map(([label, to]) => (
                  <Link key={to} to={to} className="flex items-center gap-2 text-sm text-blue-100 hover:text-black transition-colors duration-200">
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-300 shrink-0" /> {label}
                  </Link>
                ))}
              </ul>
            </div>
            ...
            <div className="lg:shrink-0">
              ...
              <motion.div className="mt-6" animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
                <Link to="apply">
                  <button className="w-full sm:w-auto px-6 py-2 border-2 border-white rounded-lg font-semibold text-sm hover:bg-white hover:text-blue-600 transition-all duration-300">
                    Apply Now →
                  </button>
          ...
          <div className="w-full lg:w-56 lg:shrink-0">
            <img src="Map.png" alt="Global Presence Map" onClick={() => setShowMap(true)} className="w-full lg:w-56 h-auto rounded-xl shadow-lg opacity-90 hover:opacity-100 transition duration-300 cursor-pointer" />
          </div>

          {showMap && (
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
              <button onClick={() => setShowMap(false)} className="absolute top-5 right-5 text-white text-3xl font-bold">×</button>
              <img src="Map.png" alt="Map Full View" className="max-w-[90%] max-h-[90%] rounded-lg shadow-2xl" />
            </div>
          )}
        </div>
      </div>
      ...
      <footer className="bg-blue-950 mt-6">
        <div className="max-w-6xl mx-auto px-4 py-4 sm:py-5 flex flex-col sm:flex-row justify-between items-center gap-2">
          <p className="text-blue-300 text-xs sm:text-sm text-center">© 2026 IIOFT. All Rights Reserved.</p>
          <div className="flex gap-4">
          ...
          <div className="flex gap-4">
            <Link className="text-blue-400 hover:text-white text-xs sm:text-sm transition-colors" to="/PrivacyPolicy">PrivacyPolicy</Link>
            <Link className="text-blue-400 hover:text-white text-xs sm:text-sm transition-colors" to="/TermsConditions">TermsConditions</Link>
          </div>
        </div>
      </footer>
